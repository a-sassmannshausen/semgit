...
(define-module (semgit conf)
  #:use-module (semgit conf getopt)
  #:use-module (semgit conf parser)
  #:use-module (semgit conf spec)
  #:use-module (ice-9 match)
  #:use-module (ice-9 vlist)
  #:use-module (semgit conf monads)
  #:use-module (semgit conf monads io)
  ...
  (apply mecha-configuration
         `(,(check-name name)
           ,(match config-dir
              ...
              (_ (throw 'config-spec
                        "CONFIG-DIR: absolute dirpatch, or #f.")))
           ...
           ,(match free-params
              ...
              (_ (throw 'config-spec
                        "FREE-PARAMS should be a list of free-params.")))
           ,(match terse
              ((and (? string?) (? (compose (cut <= <> 40) string-length)))
               terse)
              (_ (throw 'config-spec
                        "TERSE should be a string shorter than 40.")))
           ,(match long
              ((or (? string?) #f) long)
              (_ (throw 'config-spec
                        "LONG should be a string, or #f.")))
           ,(match parser
           ...
           ,(match inherit
              ((or #t #f) inherit)
              (_ (throw 'config-spec
                        "INHERIT should be #t or #f, if specified."))))))
